...
## [2026-09-08 17:48] DECISION — Phase-1 dataset re-frozen under the operator's decisions (card 008)

The operator reviewed `reports/benchmark_phase1_audit.md` (card 007) and answered its two open questions; card 008 applies them and re-derives the frozen phase-1 dataset. Nothing is fitted, no metric is computed on any partition, and the external holdout is not opened — `cycle/holdout_ledger.jsonl` is byte-identical (0 bytes) before and after.

**Decision 1 — exclude the null-skill games from the modeling population.** The 166 games with an empty historical win-rate bucket cannot be scored by R0 (whose only feature is `base_p`), so the operator ruled the exclusion lands at the point the population is defined rather than being carried into Phase 2. Derived at run time from the raw CSV (never hardcoded): those 166 games belong to **59 drafts, every one entirely null** — 0 partially-affected — so excluding games and excluding drafts are the same operation. The build **fails** (`PartiallyNullDraft`) if it ever finds a partially-null draft. Population: 241,727 → **241,561** games; 43,161 → **43,102** drafts.

**Decision 2 — take the games-played bucket set from the data.** The inherited `hist_w` map carried a `1000` key that never occurs here; it was removed. The accepted bucket set is now derived from the data (`{1, 5, 10, 50, 100, 500}`); the weight per bucket stays the inherited modeling assumption, declared as data; a bucket observed with no declared weight still fails. Card 005's structurally-absent-null carve-out is **deleted**: `deckbench.skill` now fails on any null win-rate bucket in the population (`MissingWinRateBucket`), restoring criterion 8's original intent — the branch was unreachable once the null games left, and unreachable safety behaviour is worse than none.

**Applied once, inherited downstream.** The exclusion lives in `deckbench.table` (`population_obs_ids`), at the population definition. `deckbench.identity` reads that population rather than re-deriving it; `deckbench.skill` and `deckbench.split` read the already-excluded `model_table.parquet`. The split is **recomputed** from scratch (not filtered), so the holdout fraction and the five fold sizes are correct on the new population. The criterion is a pre-draft covariate; `won` is never consulted (a test asserts the excluded set is unchanged when every outcome is flipped).

**Numbers (before → after).** dev 194,348/34,529 → **194,215/34,482**; holdout 47,379/8,632 → **47,346/8,620** (0.199991 of drafts, 0.196000 of games); dev fold sizes 39316/39067/38915/38479/38571 → **39280/39038/38902/38446/38549**; holdout time end 2026-08-29 23:39:41 → **23:33:09** (the previously-latest draft was null-skill). `mu` = **0.546211**, unchanged by construction: card 005 already computed it over the rows that carried a bucket — the same 241,561 rows now constituting the whole population — so removing the 166 null games removed exactly the rows never in the mean. Seed 20260908 unchanged; `python -m deckbench.split --verify` passes and two serial full re-derivations are byte-identical.

**Re-frozen digests (SHA256).** model_table `d9b4f5c2…`, deck_identity `2cf659d1…`, card_identity_manifest `8967247542…`, skill_features `4ac7ee8e…`, model_split `ad7f8596…`, split_manifest `6e69f9aa…`; `reports/modeling_data_audit.json` unchanged (`c1b90900…`). `data/processed/MANIFEST.sha256` regenerated over all five artifacts (repository-root-relative paths); `sha256sum -c` exits zero from the repository root. `reports/benchmark_phase1_audit.md` amended — every pinned SHA now refers to the re-frozen artifact, with an amendment section recording the decisions and the old→new counts. Full change log: `reports/phase1_refreeze.md`.

This re-freezes the phase-1 dataset under the operator's decisions so Phase 2 inherits one coherent population with no null-handling policy to invent. It does not begin Phase 2, fit any model, or open the holdout.

Refs: `reports/phase1_refreeze.md`; `reports/benchmark_phase1_audit.md`; `reports/skill_proxy.md`; `reports/split_and_seal.md`; `reports/model_table_build.md`; `src/deckbench/{table,identity,skill,split}.py`; `data/processed/MANIFEST.sha256`; `data/splits/split_manifest.json`; `docs/MTG_Deck-Strength_Modeling_Benchmark.md` (§17); `INSTRUCTIONS.md` §3; [[phase4-frozen]]

## [2026-09-08 22:30] RESULT — The evaluation panel, built before any model is scored (card 010)

Card 010 builds the metric, calibration and uncertainty panel every benchmark model will be judged by — `src/deckbench/evaluation.py` — and builds it now, before a single model has been fitted against it. This is a pre-registration device (benchmark §§9–12): a metric chosen after seeing results is a degree of freedom that can be steered toward a conclusion, so every choice is fixed in advance. The concrete failure it prevents is this repository's own `attic/haiku-2026-09-07/` `KEY FINDINGS` block — three checkmarked improvements next to a companion CI that crossed zero, with no way to adjudicate.

**The panel is a pure function of `(y, p, weight, group)` arrays.** It loads no parquet, never reads the split or the holdout, accepts no partition label, does not know which model or representation produced the predictions, and imports no learner — not `xgboost`, not `sklearn.ensemble`, nothing from `deckbench.estimator`. Tests enforce these structurally by inspecting the module's *code* (imports via `ast`, identifiers via `tokenize`), so a docstring naming a learner to say it is absent cannot trip the check. The only "fit" is the calibration intercept/slope, a two-parameter weighted logistic regression solved by hand in numpy (`_weighted_logistic_newton`); there is no learner library behind it.

**Metrics (all weighted by the passed observation weight):** log loss, Brier, RMSE, MAE, AUC (weighted Mann–Whitney, ties at half credit), and — split by outcome family — a section-9 weighted R² for **continuous** bump targets and a **Brier Skill Score** for **Bernoulli** outcomes. Each is checked against a hand-computed value on a small fixture, not against its own output. Degenerate cases asserted: perfect predictions → log loss 0 / Brier 0 / AUC 1; the weighted base-rate forecast → R² 0 and BSS 0. **The R² trap is closed:** `evaluate_metrics(..., outcome_type="bernoulli")` returns *no* `r2` key at all, only the named `brier_skill_score`; the unlabelled regression R² the quarantined pipeline reported on a raw outcome is made impossible to emit.

**Probabilities are clipped** to `[PROBABILITY_CLIP, 1−PROBABILITY_CLIP]` with `PROBABILITY_CLIP = 1e-12` (named constant, recorded in the report) before any logarithm — a confident-wrong prediction is finite, not infinite.

**Calibration.** Cox intercept/slope via weighted logistic regression on `logit(p)`; tests on an exactly-constructed over-confident set recover slope 0.5 (< 1) and an under-confident set slope 2.0 (> 1), in the known direction. Binned calibration uses **one declared strategy — equal-width bins over [0,1]** (`CALIBRATION_BINS = 10`), identical across every model (equal-width, not quantile, so the edges do not move with the model), and returns the edges alongside counts so a plot cannot silently rebin. A **smooth** curve (weighted Gaussian-kernel Nadaraya–Watson, bandwidth 0.05, 101-point grid) is provided so no conclusion rests on bin boundaries. **GAMLSS is explicitly not part of the panel.**

**Paired cluster bootstrap (§12).** Resamples `draft_id`, not rows — a test asserts every game of a sampled draft appears together, because games within a draft share a player/skill bucket/deck and resampling rows would understate the variance. **Paired:** one set of resample indices per replicate scores every model (a test with two identical models gets an exactly-zero difference in every replicate; another regenerates the indices from the seed and confirms every model saw them). Returns CIs for each model's absolute metric **and** for each pairwise **difference** (the §11 incremental-value question). **Deterministic** under `DEFAULT_BOOTSTRAP_SEED = 20260908` (two runs → identical intervals). Cluster level `draft_id` agrees with card 006's split.

**Validation.** `python -m deckbench.evaluation --selftest` exits 0; `tests/test_deckbench_evaluation.py` 33 tests pass; full suite **680 passed** in 246s (under the 300s cap); `ruff --select E,F,I,B,UP` clean and `mypy --strict src/deckbench/evaluation.py` clean. No model is fit, no metric is computed on any partition, the holdout is not opened, and the working tree is left clean (the pytest-dirtied `data/review/*.jsonl` were restored). The panel and the estimator (card 009) are decoupled in both directions: the estimator needs no panel, the panel needs no estimator.

Refs: `src/deckbench/evaluation.py`; `tests/test_deckbench_evaluation.py`; `reports/evaluation_panel.md`; `docs/MTG_Deck-Strength_Modeling_Benchmark.md` (§§9–13); `attic/haiku-2026-09-07/README.md`; [[modeling-benchmark-phase1-frozen]]

## [2026-09-09 14:30] RESULT — Which derived artifacts are actually deterministic (card 013)

`registry.md` requires that *frozen artifacts stay byte-identical and two serial builds agree*, and nothing checked it across the board. Card 013 builds the harness that decides the question for every derived artifact — `tools/determinism_audit.py` — then fixes what it finds. The harness regenerates each artifact several times, **each in a fresh process with a distinct `PYTHONHASHSEED`**, and compares the bytes the code writes. Fresh processes are load-bearing: the one real instability is `set`-iteration order, which is randomized per process; comparing inside one process fixes the seed and hides it, and comparing against git's blob invents a phantom diff under `* text=auto` (a CRLF working copy differs from an LF blob by one byte per line). Run as `python tools/determinism_audit.py --report`; it exits non-zero if any covered artifact is unstable.

**Measured before any fix (the only genuine case).** `data/review/llm_accepted.jsonl` and `llm_queued.jsonl`: two fresh regenerations (`phase3.reconcile(); phase3.finalize_faces()`, the suite's path) under different seeds gave *identical length* but *different bytes* — `llm_queued.jsonl` 37 records both runs, first diff at byte 178; `llm_accepted.jsonl` 211 records both runs, first diff at byte 1247; in both the line (record) multiset differed, i.e. a record differing internally, not records reordered. Cause: `phase3.reconcile` built `agreed_edges`, `disputed_edges`, `agreed_abilities`, `disputed_abilities` from Python `set` differences/intersections, whose iteration order is per-process random. The order of elements *inside* each record's list therefore varied.

**Fix.** Sort those set-derived keys with a total key before building the lists — edge keys are `(source, predicate, target)` string triples (naturally total, as they are unique dict keys); ability keys use a None-safe total order (`_ab_order`). Records are untouched; only their internal element order is now fixed. Verified: byte-identical across five hash seeds, and the canonical (order-insensitive) record multiset is identical to the committed version (no record added, dropped, or altered). The committed `llm_accepted.jsonl` / `llm_queued.jsonl` were rewritten once to the now-deterministic ordering so the suite leaves the tree clean.

**A fourth wrong guess, corrected by measurement.** The card recorded `card_pair_projection_audit_repair.jsonl` as unsorted / "stable only by luck". Measured, `audit_repair._write` sorts every record by `(source_card, target_card, relation)` and `add_pair` de-duplicates on exactly that triple, so the key is *total* and its upstream iterations are `sorted(...)`. Three fresh regenerations are byte-identical; `audit_repair.py` needed no change. This is the fourth mechanism in this area inferred wrong from something other than a byte comparison — the reason the deliverable is a harness, not a list of repairs.

**Coverage.** 69 derived artifacts regenerated (14 jobs, all additive layers, the phase-3 review chain, the schemas, and `ports --all` — never the destructive bare `ports`); all byte-stable across seeds. The 8-file frozen base is verified byte-identical to `frozen_manifest.json` (never regenerated). The 5 modeling-arm `data/processed` artifacts are verified against `MANIFEST.sha256` (owned by their own cards, not rebuilt here). 39 remaining derived files (effect-semantics overlay, network/community, hand-authored inputs, narrative reports, `assembly_review.jsonl` which rides with the frozen base) are reported `not_regenerable` with the reason. Full table in `reports/determinism_audit.md`.

**Validation.** `python tools/determinism_audit.py --report` exits 0; `tests/test_determinism_audit.py` 10 tests pass in ~21s; `python -m hobkg.cli ports --validate` rc 0; `ruff --select E,F,I,B,UP --line-length 100` clean on both new files; `mypy --strict tools/determinism_audit.py` clean. The harness snapshots and restores the working tree, leaving it byte-identical to how it started.

Refs: `tools/determinism_audit.py`; `tests/test_determinism_audit.py`; `reports/determinism_audit.md`; `src/hobkg/phase3.py` (`reconcile`, `_ab_order`); `registry.md` (Success Criteria); `reports/suite_stability.md`; [[repo-side-effect-hazards]]

---

## [2026-09-09 18:05] CORRECTION — Skill proxy `mu` computed per draft, matching the reproduced R implementation (card 014)

`mu`, the shrinkage target in the skill proxy `base_p` (`deckbench.skill`), was the mean of `base_p_raw` over **games**. The R implementation this proxy reproduces (`scripts/R/04_real_inference_refactored.R`) builds its data frame `x` at draft/event level and takes `mu <- mean(x$base_p_raw)` at **line 324** — a mean over drafts, one value per draft. Benchmark section 3 requires the proxy be *reproduced, not improved*, so that the target-formulation axis (T0/T1/T2) is isolated from changes in skill estimation; a per-game `mu` is not the proxy the earlier work used, so the implementation quietly failed that requirement. Card 014 changes ours to a per-draft mean and carries the change through everything that depends on it.

**This is a fidelity correction, not an improvement.** A per-game mean over-samples strong players — under the Arena 7-wins/3-losses run structure a good run is up to ten games and a bad one three, so game count rises with win rate (measured: mean games/draft climbs from 4.46 in the `<0.45` win-rate range to ~6.65 in `0.66–0.86`). Averaging per draft gives each draft equal weight, which is what line 324 does. Nothing about the learner, `lambda`, the `hist_w` map, the clip bound, or the accepted bucket set changed; only the population `mu` averaging changed. `base_p` is not described as skill anywhere.

**The numbers.** `mu` 0.546211 (per game) → **0.533339** (per draft), difference **+0.012872**. `base_p_raw` is unchanged for every row; only `base_p` moves, down by at most ~0.011 and largest exactly where the proxy leans hardest on `mu` (small `hist_w`, i.e. near-new players): mean Δ`base_p` by games bucket ranges from −0.0090 (bucket 5) to −0.0037 (bucket 500). The modeling population is untouched — the same **241,561 games across 43,102 drafts** card 008 froze, no row added, dropped, or reweighted. The per-draft value is well defined because the historical win-rate bucket is constant within every draft (audit: non-constant in 0 drafts); `deckbench.skill` now stops the run (`InconsistentWithinDraftWinRate`) if any draft ever presents two buckets.

**The cascade, and where it stops.** `skill_features.parquet` re-emitted (SHA256 `4ac7ee8e…` → **`8e86df3a…`**); `data/processed/MANIFEST.sha256` regenerated (root-relative paths; `sha256sum -c` passes from the repo root) — only the skill member moved; the other four (`model_table`, `deck_identity`, `card_identity_manifest`, `model_split`) are byte-identical, and the manifest stays the canonical five-member `card-004/005/006` form. **Fixed a latent manifest bug while here** (likely the prior card-014 gate-decline's root cause): `deckbench.skill` wrote only its own four members, so re-emitting the proxy dropped card 006's `model_split.parquet` from the integrity manifest and dirtied the tree against the committed five-member manifest. `deckbench.skill.manifest_members` now includes `model_split.parquet` **when it exists**, writing the five-member manifest byte-identically to what `deckbench.split` writes (verified) — on a fresh build the split does not yet exist and the four-member manifest is still correct, then `deckbench.split` adds the fifth. Card 011's T0 fits were **refitted** through `deckbench.estimator.fit_and_predict` unchanged (same seed 20260908, same frozen split): R0 is `[base_p]` and R1 contains it, so both were stale the moment `mu` moved. The recorded `split_sha256` is still `ad7f8596…`, matching `split_manifest.json`; the chosen hyperparameters and round counts shifted with the new feature (R0 325→367 rounds; R1 to a different grid point, 194→267). `model_split.parquet` is keyed on `draft_id`/`draft_time`, not `base_p`, so its hash is unchanged and no re-split was needed. **Why now:** T1 is the bump against this baseline and T2 the bump against a learned one; if `mu` moved after they were fit, both would be void. The cost today is one re-emission and two refits; after T2 it is every fit in the phase.

**Not concluded, and not opened.** No comparison between R0 and R1 is drawn from the refitted development metrics — card 011's prohibition stands unchanged and for the same reason (they are contaminated by the folds that selected the hyperparameters). The holdout was **not** opened; `cycle/holdout_ledger.jsonl` is byte-identical (0 bytes) before and after. The determinism audit still reports zero unstable artifacts.

**A section-7 item closed alongside it.** `reports/benchmark_phase1_audit.md` § 7 item 4 flagged 15,680 non-modal deck sizes as uncharacterised. Measured: **13,424 are 41-card decks (5.55% of rows)** — an ordinary Limited choice of one extra card — everything from 42 upward is **2,256 rows (0.93%)**, and 60-card decks number five. This is a normal deck-size distribution, not an anomaly; the item is recorded as **closed**.

**Validation.** `python -m deckbench.skill` rebuilds byte-identically; `python -m pytest tests/test_deckbench_skill.py -q` passes (26 tests, incl. the new invariance test — one draft many games, another one game, `mu` = the unweighted mean of the two drafts' values — the within-draft-inconsistency guard, and the five-member-manifest guard that keeps `model_split` pinned); `sha256sum -c data/processed/MANIFEST.sha256` OK; `python tools/check_append_only.py LABNOTEBOOK.md` exits 0; `mypy --strict src/deckbench/skill.py` clean; `ruff --select E,F,I,B,UP --line-length 100` clean on the touched files; `python tools/determinism_audit.py --report` reports zero unstable artifacts.

Refs: `reports/mu_fidelity_correction.md`; `src/deckbench/skill.py`; `tests/test_deckbench_skill.py`; `reports/skill_proxy.md`; `reports/t0_development_fits.md`; `reports/benchmark_phase1_audit.md` (§ 0.1, § 5 claim 4, § 7 items 4 & 6); `data/processed/MANIFEST.sha256`; `data/runs/T0_R0_run.json`, `data/runs/T0_R1_run.json`; `scripts/R/04_real_inference_refactored.R` (line 324); `docs/MTG_Deck-Strength_Modeling_Benchmark.md` (§ 3); [[modeling-benchmark-phase1-frozen]]

## [2026-09-10 11:20] CORRECTION — Run records named the wrong xgboost, and the T0 fits were not reproducible

`data/runs/T0_R0_run.json` and `T0_R1_run.json`, written by card 011 and rewritten by card 014, recorded `"xgboost_version": "3.4.1"`. The boosters they describe embed **3.1.2**. `reports/t0_development_fits.md` carried the same false string. Both survived a reviewer PASS twice.

**How a wrong value got written by correct-looking code.** `deckbench.estimator` filled the field from `str(xgb.__version__)` — the *Python package's* self-report, which is not the version of the compiled `libxgboost` that actually trains. On 2026-09-09 a 3.4.1 wrapper sat over a 3.1.2 library, so the run record and the booster written 1 ms apart in the same call disagreed. Nothing was hand-edited; the code did exactly what it said, and what it said was the wrong source. A past session even checked and recorded "the default venv has xgboost 3.4.1 (matching the run records)" (`CONVERSATION_LOG.md` line 128474) — the wrapper and the record agreed with each other precisely because both came from the same wrong source, which is why cross-checking a self-report against a self-report cannot catch this.

**Detection.** Comparing the run record against the *artifact* rather than against another self-report: an xgboost booster embeds the version that wrote it, readable via `save_raw(raw_format="json")["version"]`. All four boosters on disk read `[3, 1, 2]`.

**The part that is not bookkeeping.** Refitting under a matched 3.1.2 wrapper did **not** reproduce the card-011/014 T0 artifacts. All four T0 files changed. `T0_R0`'s grid search selected a different point — `max_depth` 3→4, `subsample` 1.0→0.8, `colsample_bytree` 1.0→0.8, **367→136 rounds** — and the panel metrics moved in the fifth decimal (`log_loss` 0.666435→0.666441, `cal_slope` 0.990676→0.994568). `T0_R1` moved to 268 rounds at the same grid point; `log_loss` 0.664434→0.664381. The **control is clean**: card 015's T1 fits, produced under a matched wrapper, reproduced **byte-identically** — all six files, hash for hash — and a second consecutive T0 refit reproduced the new T0 exactly (identical grid point, rounds, and metrics to six decimals). So this was never a determinism failure: card 013's guarantee holds *within* an environment. The environment itself moved mid-project, unpinned and unrecorded, and the run records could not show it because they were recording the wrong field.

**Why this mattered before card 017 and not after.** The holdout read is a single irreversible event comparing T0 against T1 (and T2). Had it run on the old artifacts, the comparison would have rested on T0 models nobody could rebuild — one arm of the contrast fitted by a wrapper that no longer exists on this machine, the other by the current one. That is a learner difference sitting inside an axis the benchmark (§8) requires to hold the learner constant. The cost of catching it now is two refits; after 017 the ledger line is spent.

**The fix.** `xgboost_version` is now read from the library's own `XGBoostVersion()` C entry point, and the wrapper's self-report is kept beside it as `xgboost_python_version` rather than discarded — the Python side builds the DMatrix and drives the boosting loop, so it can move results too, and a disagreement between the two fields is itself the signal that caught this. It **fails closed** (`ProvenanceUnavailable`) if the library will not report its version, because a plausible placeholder is what this entry is about. Both reports print both versions.

**A second instance of the same class.** Regenerating `reports/t0_development_fits.md` silently deleted card 014's `mu`-correction banner: that paragraph had been hand-added to the committed file and the generator never knew about it, so it survived only until someone reran the fit. Same defect shape as the version string — information in a generated artifact that its generator cannot reproduce. It is now a **Regeneration history** section inside `deckbench.targets`, covering both the card-014 `mu` correction and this one, so the report regenerates complete instead of decaying.

**Tests, because nothing tested this.** Three added to `tests/test_deckbench_estimator.py`: a monkeypatched wrapper claiming `99.99.99` must not reach `xgboost_version` (and must appear in `xgboost_python_version`); the recorded version must equal the version embedded in the booster just written; and every tracked run record must agree with its booster on disk, which is the check that would have caught the original defect because it compares a record against an artifact, not against a self-report.

**Not concluded, and not opened.** No comparison between R0 and R1, or between T0 and T1, is drawn from the refitted development metrics; card 011's prohibition stands. The holdout was **not** opened — `cycle/holdout_ledger.jsonl` is 0 bytes before and after. `data/processed/MANIFEST.sha256` verifies unchanged (all five members OK); no phase-1 artifact was touched, and the split hash is still `ad7f8596…`.
...
## [2026-09-11 05:40] RESULT — Run-level calibration, and the deck signal is attenuated (derived analysis, no read spent)

A re-expression of card 017's completed measurement at the unit a Limited player actually experiences — the **run**, an Arena event that ends at 7 wins or 3 losses — rather than the game. **No holdout read was spent.** Every outcome comes from the prediction artifacts card 017 committed, which exist precisely so its numbers can be re-expressed without reopening the seal; `draft_id` is joined from the frozen `model_split.parquet` as grouping metadata only. `load_holdout` is never called, `repeat=True` appears nowhere, and `cycle/holdout_ledger.jsonl` is unchanged at one line before and after. Reproducible: `python tools/run_level_calibration.py`.

The distinction being held: re-expressing one confirmed effect in different units is not a second probe. Bootstrapping metric after metric on a read partition until something clears zero would be, and the tool computes a **declared, fixed** set of quantities rather than searching.

**Unit.** 8,620 holdout runs, 47,346 games, mean 5.49 games per run. Predicted run win rate is the mean of the per-game probabilities; observed is wins/games; fits are WLS weighted by games. All intervals are the paired cluster bootstrap over drafts at the benchmark's declared settings (1000 replicates, seed 20260908, 95% percentile) through `evaluation.bootstrap_replicate_indices` — the same primitive card 017 used.

| quantity | point | 95% CI | excludes null |
| --- | --- | --- | --- |
| calibration slope, skill only (`T2_R0`) | 0.9774 | [0.9386, 1.0181] | no (null 1) |
| calibration slope, skill + deck (`T2_R1`) | 0.9859 | [0.9494, 1.0228] | no (null 1) |
| calibration slope, difference | +0.0085 | [-0.0090, +0.0239] | no (null 0) |
| **attenuation slope** | **1.1782** | **[1.0355, 1.3156]** | **yes (null 1)** |

**Finding — the model understates the deck effect.** Regressing the residual the *skill-only* model left on the *deck* model's adjustment gives slope **1.178 [1.036, 1.316]**, with 99.2% of bootstrap draws above 1. A slope of 1 would mean the deck signal is scaled exactly right; above 1 means the realised effect exceeds the predicted one, by roughly **17.8% [3.5%, 31.6%]**. This is the expected signature of gradient boosting with early stopping — regularisation shrinks predictions toward the mean and the shrinkage surfaces as attenuation. The interval is wide: "the model understates the deck effect" is supported; "by 18%" is a point estimate that should not be quoted as precise.

**Robustness.** The slope is 1.1782 at game level unbinned (47,346 rows), 1.1839 at run level unbinned (8,620 rows, weighted), and 1.1981 on the twenty ventile means. Binning inflates it about 2%, the expected direction and negligible; aggregation changes almost nothing. The intercept is identical to five decimals at game and run level (-0.00629), which is the weighted means being preserved under the collapse. **The reported fits are on unbinned rows; ventiles are display only.**

**The attenuation is not a disguised skill correction.** A concern is that `observed - predicted_skill_only` also carries the skill model's own error, so a deck model quietly repairing skill-side miscalibration would inflate the slope. Measured correlation between the deck adjustment and the skill-only prediction is **+0.038 (game), +0.045 (run)** — almost no shared variance for that route — and the skill-only model is itself well calibrated.

**A claim withdrawn before it reached the report.** An intermediate version of this analysis presented "adding the deck improves calibration" as a finding, on the point estimates (slope 0.9774 to 0.9859, intercept +0.0071 to +0.0015). The bootstrap interval on that difference is **[-0.0090, +0.0239]**, squarely including zero. It is noise. What the calibration numbers *do* support is that **both** models are well calibrated at run level — both intervals bracket 1, and the ventile points track the diagonal from 31% to 75% predicted win rate with a maximum single-bin deviation of 2.2 points.

**Whole-run translation, and why it needs care.** A run is *stopped*, so games played is a random variable and a per-game probability cannot be multiplied by an average length. Under independence at constant `p`, `P(finish 7-L) = C(6+L,L) p^7 q^L` for `L = 0,1,2` and `P(finish W-3) = C(W+2,2) p^W q^3` for `W = 0..6`. That model puts the trophy rate at **8.98% for a 50% player**, matching the observed Arena figure — a useful check. Because the trophy carries `p^7`, one percentage point of `p` moves it by roughly 1.5 to 2 points in this range. Applied to the holdout predictions, a bottom-decile against a top-decile deck (-0.037 and +0.037 on `p`, from the median run's 0.5787) spans **3.30 to 4.13 expected wins** and **13.80% to 26.17% trophy rate**. This is **indicative, not measured**: a deterministic transform of predictions, with no interval, assuming within-run independence, and inheriting the attenuation above — so the true gap is likely *wider* than shown.
...
## [2026-09-11 17:30] RESULT — Phase 3 exploration: card identity already extracts what the knowledge graph can name

Full write-up in `reports/phase3_exploration.md`; simulator preserved as `tools/game_script.py`. **Exploratory, development rows only, no holdout read spent** — `cycle/holdout_ledger.jsonl` still records exactly one line, by card 017. No card was run. Every number is contaminated by the folds that selected the models' hyperparameters. The purpose was to decide *what phase 3 should card*, before carding anything, and the answer is a negative.

**The headline.** Two mechanics chosen to span the structural extremes were both fully captured by card identity, with the residual at the noise floor in each case. The simplest game script does not beat the skill proxy alone. **Phase 3 should not be carded as an aggregate-prediction bet.**

**The arms connect.** All **193 of 193** modelling features join to the frozen HOB graph via `card_ports.jsonl` face names. R2 is buildable; that was never the obstacle.

**R2 candidates, and what survived contact.** Ports (typed `produces`/`consumes`/`costs`/keywords) are usable and set-independent. Module liveness **failed as designed** — module `consumers` are mostly edges with `card: None`, so "consumers ∩ deck" is empty by construction and every module reads dead. Raw pair density is **heavily contaminated**: of 35 relation types the high-count ones fire for any (spell, creature) or (land, spell) pair — one real deck showed `Bilbo's Deadly Slice --CAN_DESTROY-->` all eight of its own creatures, and `INFRASTRUCTURE_CASTING(62)` meaning "you have lands" — so the density is largely a proxy for creature and land counts that R1 already encodes. The anti-relabeling guard is concrete and was needed: **61 of 151 port types appear on exactly one card**, and a port type carried by one card *is* that card's indicator. Support ≥5 cards leaves 54; the prototype phi_KG was 70 dimensions against R1's 193.

**R3, the game script — built and null.** A Monte Carlo mana simulator; cards reduce to (pips, generic, land/produces) and what they *do* is ignored. Settled policy after four iterations: on the play, no mulligans, **tapped lands modelled** (every dual in this set enters tapped, the basics do not), and casting is **exact within the turn** rather than greedy — the best subset by (spell count, then mana spent), since several spells in one turn only require the *combined* cost to be payable. Output is 10 turns x 9 statistics = **90 dimensions, but a participation ratio of 7.3** (3 PCs explain 71.1%, 10 explain 95.0%). **It uses no graph at all**, which makes it the correct *control* for phase 3 rather than a candidate.

Fitted on development across all 53,116 distinct decks with the frozen folds and one fixed hyperparameter point: R0 skill-only **0.037551** Brier skill; R1 skill+identity **0.041720**; R3 skill+script **0.036017**; R1+R3 **0.040860**. **R3 is worse than skill alone, and adding the script to identity makes identity worse.** Retuning R3 four ways (depth 2-5, 60-268 rounds) reached a best of 0.036841, still below R0.

**The mechanic grids show a real interaction that identity has already learned.** The KG names three second-draw payoffs sharing an identical 25-card enabler set (24.5% of games). Win rate over (payoff copies x enabler copies) is a clean interaction surface: row 0 flat, every payoff row climbing, **crossover at ~7 enablers** for all three payoffs independently, and a dose-response that *reverses sign* — at 3 enablers more copies is worse, at 12 more copies is better. Difference-in-differences per enabler: Apothecary +1.271%, Bard +1.091%, Councillors +0.852%. **The gradient survives skill stratification** (pooled 5.38pp; quintiles 5.46 / 6.50 / 4.41 / 4.01 / 5.44).

Bothersome Noisemaker was chosen as the diffuse counter-case — *"whenever you cast a noncreature spell"*, 81 enablers, essentially every noncreature spell. Opposite risk profile: row 0 **dead flat** across 158,000 games, an **upside** mechanic reaching +6.3pp where second-draw's best cell was +1.0pp, milder downside (a 2-mana 2/2 is playable when the trigger rarely fires; Apothecary without drawers is blank text). Gradient survives stratification again (pooled 3.18pp).

**And identity has both.** Partition variance against the residual after `T0_R1`'s out-of-fold predictions:
...
The second-draw partition alone is worth ~51% of the whole measured deck effect — but four-fifths is already in R1's predictions and the remainder is indistinguishable from cell-estimation noise.

**Netting out skill properly changes the story.** `base_p` is shrunk toward mu, so strong players overperform it by construction (`corr(skill, won - base_p) = +0.716` across cells). Substituting `m_hat` — T2's calibrated cross-fitted baseline — the bump range goes from [-9.9, **+10.1**] to [-12.1, **+5.8**]: **the upside roughly halves and the downside deepens.** Most of the apparent reward for building second-draw well was good players being under-rated by the proxy. With a calibrated baseline the mechanic is **mainly a downside risk** — assembling it correctly is worth +0.32pp over not playing it, while stranding the payoff costs -5.22pp. The correlation falls to +0.586 but cannot reach zero: `m_hat` is calibrated against `base_p`, not true skill, and **this dataset has no player identifier**.

**What the graph has that identity cannot.** `ENABLES_TRIGGER` is directed and overwhelmingly so — **441 edges, 2 reciprocated (0.5%)**. `Plunder the Trollshaws -> Lakeshore Apothecary` exists; the reverse does not. The outcomes respect the arrow: payoff-without-enablers costs **-5.22pp**, enablers-without-payoff **-2.43pp**, a **2.1x asymmetry** that a symmetric `payoff x enablers` feature cannot represent. Four things the graph uniquely supplies and the current benchmark does not measure: direction, role labels that transfer to a new set (identity's 193 columns cannot transfer at all), the feasible-space map, and causal structure for phase 4.

**The feasible-space observation, which bounds card 017.** The empty grid cells are not missing data — nobody builds 4 payoffs with 0 enablers, because the community recognises it as non-viable. That is a positivity violation whose absence is itself information, and it means the measured ~13% deck contribution was estimated on **decks that passed a human viability filter**. The honest statement is "among decks people actually choose to build, decks contribute 13%" — a sharper caveat than any listed in section 13.

**Three predictions of mine that measurement falsified**, recorded because the reasoning was plausible each time and a future session will be tempted by it. (1) "The model is distracted by skill" — H2's premise; trees learn a smooth 1-D curve nearly for free, so there was no capacity to liberate, and H2's null was predictable in advance. (2) "Identity recovers only two-thirds of the script, so the missing third is R3's opportunity" — the missing third does not predict winning; bounding an opportunity is not establishing its value. (3) "Diffuse mechanics will defeat identity" — the diffuse mechanic was captured *better*. The consistent error is **underestimating what a 268-tree ensemble extracts from 194,000 games**: reasoning about what trees represent well structurally neglects that with this much data they approximate a great deal badly-but-adequately.

**Recommendation.** Do not card R2 against incremental Brier; the evidence says it loses. If phase 3 proceeds it should be judged on what the graph actually supplies — stratified evaluation (does the KG identify *where* deck composition matters?), transfer to another set, or causal structure. Section 11 frames everything as incremental aggregate performance and that instrument is measuring the wrong thing for this arm. Two items on the docket, not pursued: the **dead-zone subpopulation** (where the graph says a mechanism is dead and people built it anyway, the deck effect should be far larger than 13%), and the **reachability script** `phi_script(D, G)` as the spec actually defines it — the only representation examined that R1 could not approximate in principle.

Refs: `reports/phase3_exploration.md`; `tools/game_script.py`; `data/graph_global/{card_ports,pair_index,mechanism_modules}.jsonl`; `data/runs/T0_R1_predictions.parquet`, `T0_R0_predictions.parquet`; `docs/MTG_Deck-Strength_Modeling_Benchmark.md` (sections 11, 13, R2, R3); LABNOTEBOOK entries [2026-09-11 04:11] (the holdout read) and [2026-09-11 05:40] (run-level calibration); [[modeling-benchmark-phase1-frozen]] [[phase4-frozen]]
