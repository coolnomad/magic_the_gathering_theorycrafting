...
**Closed retrospectively on 2026-09-09.** This card sat in `REVIEW` from
2026-09-03 to 2026-09-09 -- the only card in the project never to reach a
terminal state -- because its reviewer step was never run, not because anything
was outstanding. Its work is complete and verified: `tests/test_ports.py` passes
75 tests, and `hobkg.cli ports --validate` reports 210 ports, 2242 edges, 0
unresolved abilities, 0 undeclared concepts and 0 selector-in-name hits.

**The acceptance criteria above are deliberately left unchecked**, because
several are no longer true as written and ticking them would be false. In
particular, the criterion requiring `card_ports.jsonl` to carry "exactly 11
records" was correct for the pilot and was superseded by card 002, which
extended the derivation to all 210 faces without adding per-card branches. The
anti-transcription criteria -- no face id, no card name, no per-card branch in
`src/hobkg/ports.py` -- do still hold and are enforced by live tests.

Read the criteria as the contract the pilot was held to, and card 002 as what
the layer became. The distinction is worth preserving rather than editing away.
