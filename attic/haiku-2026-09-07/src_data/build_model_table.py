...
# We know from data audit there are 193 deck_* columns
# Load column names from first read attempt
def get_columns_safely():
    ...
    try:
        ...
        log.info("Using hardcoded column list (193 cards)...")
        # Fallback: generate column names based on known structure
        # This is based on the data audit
...
def load_and_aggregate():
    ...
    # Read in very small chunks to minimize memory
    chunk_size = 5000
...
def compute_skill_proxy(draft_dict):
    ...
    for draft_id, data in draft_dict.items():
        ...
        hist_w = float(games_bucket_to_weight.get(n_games_bucket, 1))
